...
export function LanguageSwitcher({ compact = true }: { compact?: boolean }) {
  ...
  const current = (i18n.resolvedLanguage as Lang) ?? "de";

  return (
    <div className="inline-flex items-center gap-1.5 rounded-md border bg-card h-9 px-2" title={t("common.language")}>
      ...
      <Select value={current} onValueChange={(v) => i18n.changeLanguage(v)}>
        <SelectTrigger className="h-7 border-0 shadow-none px-1 focus:ring-0 text-xs font-medium w-[58px]">
          <SelectValue>{compact ? SHORT[current] : LABELS[current]}</SelectValue>
        </SelectTrigger>
